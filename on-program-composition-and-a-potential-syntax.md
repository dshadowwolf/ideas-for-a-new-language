...
### On Re-binding Execution Contexts and Issues With Such
After a lot of discussion with several other hackers (see [the jargon file](http://www.catb.org/jargon/html/H/hacker.html), all but definition 8) it has become clear that there are a lot of issues with the way any specifically named block of code can be re-bound to a new execution context. One of the main issues was that a function or method might rely on data from within the variable scope of the execution context in which it was defined - without some means of the system tracking this the binding to a different execution context could cause major issues, including values that are being relied on no longer existing. At first it was thought that an auxiliary language was going to be needed to define what any given function or method required for operation, which led to this, specific, formulation of Turned-C being set aside for a long time.

But, as often happens, my mind was working overtime as I tried to sleep one night and it hit upon a solution. In ECMAScript you can change the context of a method or function and it will still work, meaning it carries some information about the scope and context in which it was defined with it through the re-parenting. Though this seems complex, the basic concept isn't - the idea is that an underlying "weak binding" is created for each function and method at the location where they are first defined (and ***ONLY*** at this point - no context information is added to this weak binding, regardless of how many more times or locations it might be bound at). That weak binding is then used as a base onto which the scope and excution context of the newly bound location is layered, giving the function/method a way to maintain those values that it might rely on while also allowing the new binding location to override them as needed.
